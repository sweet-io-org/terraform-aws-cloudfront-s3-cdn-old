...
resource "aws_s3_bucket" "origin" {
  ...
  dynamic "lifecycle_rule" {
    for_each = var.lifecycle_enabled ? var.lifecycle_policy : []
    ...
    content {
      ...
      expiration {
        days = lookup(lifecycle_rule.value.expiration, "days", 7)
      }
    }
  }
...
resource "aws_cloudfront_distribution" "default" {
  ...
  lifecycle {
    ignore_changes = [
      ...
      ordered_cache_behavior[1].lambda_function_association,
      ordered_cache_behavior[2].lambda_function_association,
      ordered_cache_behavior[3].lambda_function_association,
    ]
  }
}
